...
# 1. Просто бронируем товар
async def handle_reserve(request):
    try:
        data = await request.json()
        item_id, user_id, username = data.get('item_id'), data.get('user_id'), data.get('username', 'Unknown')

        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("SELECT name FROM items WHERE id = %s AND status = 'Доступен'", (item_id,))
                item = cur.fetchone()
                if item:
                    cur.execute("UPDATE items SET status = 'Забронирован', buyer_id = %s, reserved_at = %s WHERE id = %s",
                                (user_id, datetime.now(), item_id))
                    conn.commit()
                    # Просто уведомление админу без кнопок
                    await bot.send_message(ADMIN_ID, f"⏳ Новая бронь из Web App!\n👤 @{username}\n📦 {item['name']}")
                    return web.json_response({"success": True, "requisites": PAYMENT_REQUISITES}, headers={"Access-Control-Allow-Origin": "*"})
                return web.json_response({"success": False, "error": "Товар уже занят"}, headers={"Access-Control-Allow-Origin": "*"})
    except Exception as e:
        logging.error(f"Ошибка брони: {e}")
        return web.json_response({"success": False}, status=500, headers={"Access-Control-Allow-Origin": "*"})

# 2. Вызывается при нажатии "Проверить оплату"
    ...
    try:
        ...
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                item = cur.fetchone()
                if item:
                    # Теперь присылаем кнопки подтверждения
                    ...
                    await bot.send_message(ADMIN_ID, f"💰 **Юзер заявляет об оплате!**\n👤 @{username}\n📦 {item['name']}",
...
async def admin_pay_reject(callback: types.CallbackQuery):
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("UPDATE items SET status = 'Доступен', buyer_id = NULL, reserved_at = NULL WHERE id = %s", (item_id,))
            conn.commit()
    ...
    with get_db_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            if item:
                ...
                conn.commit()
                # Для текстового каталога сразу шлем подтверждение (так проще)
                admin_kb = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"pay_yes_{item_id}_{user.id}"),
                     InlineKeyboardButton(text="❌ Отклонить", callback_data=f"pay_no_{item_id}_{user.id}")]
                ])
                await callback.message.answer(f"✅ Забронировано: {item['name']}\n\n{PAYMENT_REQUISITES}")
                await bot.send_message(ADMIN_ID, f"💰 Бронь из каталога: @{user.username}", reply_markup=admin_kb)
    await callback.answer()

var RLottie = (function () {
  ...
  function initApi(callback) {
    if (apiInited) {
      ...
      if (!apiInitStarted) {
        ...
        QueryableWorkerProxy.init(new URL('tgsticker-worker.js?v=8.9-opt.50', document.baseURI).href, rlottie.WORKERS_LIMIT, function() {
          apiInited = true;
  ...
  function destroyPlayer(el) {
    ...
    if (rlPlayer.workerProxy && rlPlayer.workerProxy.destroy) {
      rlPlayer.workerProxy.destroy();
    }
    delete el.rlPlayer;
    delete rlPlayer;
    setupMainLoop();
  ...
  RawPlayer.prototype.loadFromUrl = function(url) {
    ...
    var self = this;
    var workerUrl = new URL('tgsticker-worker.js?v=8.9-opt.50', document.baseURI).href;
...
var QueryableWorkerProxy = (function() {
  ...
  Proxy.prototype.destroy = function() {
    for (var i = 0; i < this.items.length; i++) {
      var item = this.items[i];
      item.worker.sendQuery('destroy', item.reqId);
      delete wrMap[item.reqId];
    }
    this.items = [];
    this.itemsMap = {};
    delete proxies[this.proxyId];
  };
